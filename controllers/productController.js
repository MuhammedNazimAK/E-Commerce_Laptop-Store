...
const addProduct = async (req, res) => {
  try {
    ...
    if (req.files && req.files.images) {
      const imagesToUpload = Array.isArray(req.files.images)
        ? req.files.images
        : [req.files.images];
      ...
      if (imagesToUpload.length > 0) {
        try {
          ...
          return res
            ...
            .json({ success: false, message: 'Error uploading new images' });
        }
      }
    }
    ...
    if (req.body.existingImages) {
      ...
      try {
        ...
        return res
          ...
          .json({ success: false, message: 'Invalid existing images data' });
      }
    }
...
const loadProductListingPage = async (req, res) => {
  try {
    ...
    const processors = await Product.distinct(
      'technicalSpecification.processor'
    );
    ...
    const graphicsCards = await Product.distinct(
      'technicalSpecification.graphicsCard'
    );
    ...
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .send('An error occurred while loading the product listing page');
  }
};
...
const updateProduct = async (req, res) => {
  try {
    ...
    if (req.body.categories && req.body.categories.length > 0) {
      try {
        parsedCategories = JSON.parse(req.body.categories).map(
      ...
      { _id: productId },
      {
        $set: {
          ...
          'basicInformation.description':
            updatedData.basicInformation.description,
          'designAndBuild.color': updatedData.designAndBuild.color,
          'technicalSpecification.processor':
            updatedData.technicalSpecification.processor,
          'technicalSpecification.ram': updatedData.technicalSpecification.ram,
          'technicalSpecification.storage':
            updatedData.technicalSpecification.storage,
          'technicalSpecification.graphicsCard':
            updatedData.technicalSpecification.graphicsCard,
          'pricingAndAvailability.regularPrice':
            updatedData.pricingAndAvailability.regularPrice,
          'pricingAndAvailability.salesPrice':
            updatedData.pricingAndAvailability.salesPrice,
          'pricingAndAvailability.stockAvailability':
            updatedData.pricingAndAvailability.stockAvailability,
  ...
  try {
    ...
    const result = await getCachedData(cacheKey, async () => {
      ...
      if (filters.processors.length > 0) {
        matchStage.$match['technicalSpecification.processor'] = {
          $in: filters.processors,
        };
      }
      if (filters.graphicsCards.length > 0) {
        matchStage.$match['technicalSpecification.graphicsCard'] = {
          $in: filters.graphicsCards,
        };
      }
      ...
      switch (sort) {
        ...
        case 'priceAsc':
          aggregationPipeline.push({
            $sort: { 'pricingAndAvailability.salesPrice': 1 },
          });
          break;
        case 'priceDesc':
          aggregationPipeline.push({
...
const productOfferController = {
  getProductOffersPage: async (req, res) => {
    try {
      ...
      res
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
        .render('users/pageNotFound', { message: 'Internal server error' });
    }
  },
  ...
  createProductOffer: async (req, res) => {
    ...
    try {
      const { offerName, product, discountPercentage, startDate, endDate } =
        req.body;
      const isDefault =
        req.body.isDefault === true || req.body.isDefault === 'on';
      ...
      if (isDefault) {
        ...
        if (existingDefault) {
          ...
          return res.status(StatusCodes.CREATED).json({
            success: true,
            message:
              'A default offer already exists. New offer created as inactive.',
            offer: newOffer,
          });
        }
      } else {
        if (!product) {
          return res
            .status(StatusCodes.BAD_REQUEST)
            .json({ message: 'Product ID is required for non-default offers' });
        }
        ...
        if (overlappingOffer) {
          return res
            .status(StatusCodes.BAD_REQUEST)
            .json({
              message: 'An overlapping offer already exists for this product',
            });
        }
      }
  ...
  updateProductOffer: async (req, res) => {
    ...
    try {
      ...
      if (!offer) {
        return res
          .status(StatusCodes.NOT_FOUND)
          .json({ message: 'Product offer not found' });
      }
      ...
      if (isDefault !== undefined && isDefault !== offer.isDefault) {
        ...
        if (!confirmResult.isConfirmed) {
          return res
            .status(StatusCodes.BAD_REQUEST)
            .json({ message: 'Default status change cancelled' });
        }

        if (isDefault) {
          const existingDefault = await ProductOffer.findOne({
            isDefault: true,
          ...
          if (existingDefault) {
            return res
              .status(StatusCodes.BAD_REQUEST)
              .json({ message: 'Another default offer already exists' });
          }
        }
      }
      ...
      if (!isDefault && product) {
        ...
        const overlappingOffer = await ProductOffer.findOne({
          ...
          $or: [
            {
              startDate: { $lte: endDate || offer.endDate },
              endDate: { $gte: startDate || offer.startDate },
            },
            {
              startDate: {
                $gte: startDate || offer.startDate,
                $lte: endDate || offer.endDate,
              },
            },
            {
              endDate: {
  ...
  loadAddProductOfferPage: async (req, res) => {
    try {
      ...
      res
        ...
        .render('admin/product-offer-add', { products });
    } catch (error) {
  ...
  loadEditProductOfferPage: async (req, res) => {
    try {
      const offer = await ProductOffer.findById(req.params.id).populate(
        'product'
      );
      if (!offer) {
        return res
          .status(StatusCodes.NOT_FOUND)
          .render('users/pageNotFound', { message: 'Product offer not found' });
      }
      const products = await Product.find({});
