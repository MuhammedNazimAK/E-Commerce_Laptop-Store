...
const addAddress = async (req, res) => {
  try {
    ...
    const { name, addressType, city, landMark, state, pinCode, mobile } =
      req.body;
    ...
    res
      ...
      .send('Server error while adding address');
  }
};
...
const editAddress = async (req, res) => {
  try {
    ...
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ error: 'Server error while editing address' });
  }
};
...
const deleteAddress = async (req, res) => {
  try {
    ...
    if (result.nModified === 0) {
      return res
        .status(StatusCodes.NOT_FOUND)
        .json({ success: false, message: 'Address not found or not deleted' });
    }
    ...
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ error: 'Server error while deleting address' });
  }
};
...
const getAddressDetails = async (req, res) => {
  try {
    ...
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ error: 'Server error while fetching address details' });
  }
};
