...
const {
  applyReferralReward,
} = require('../../public/javascript/referralService');
  ...
  try {
    ...
    const result = await getCachedData('homePage', async () => {
      ...
      const topProducts = await Promise.all(
        (await Product.find().sort({ salesCount: -1 }).limit(8)).map(
          async (product) => {
            const productWithOffers = await getProductWithOffers(product._id);
            return productWithOffers;
          }
    ...
    res
      ...
      .render('users/pageNotFound', { message: 'Error loading home page' });
  }
};
...
const renderLoginPage = (req, res) => {
  try {
    ...
    const message =
      req.session.message || req.flash('success') || req.flash('error') || '';
    ...
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .render('users/pageNotFound', { message: 'Error loading login page' });
  }
};
...
const registerUser = async (req, res) => {
  try {
    const { firstName, lastName, email, password, mobile, referralCode } =
      req.body;
...
const resendOtp = async (req, res) => {
  try {
    ...
    const emailSent = await sendVerificationEmail(email, otp);
    if (!emailSent) {
      return res.render('users/enter-otp', {
  ...
  try {
    ...
    const transformedOrders = orders.map((order) => {
      ...
      if (
        ...
        orderObj.shippingAddress.address.length > 0
      ) {
      ...
      return {
        ...
        products: orderObj.products.map((product) => ({
          ...
          product: {
            basicInformation: {
              name: product.product.basicInformation
                ? product.product.basicInformation.name
                : 'Unknown Product',
            },
          },
        })),
    ...
    res
      ...
      .json({
        success: false,
        message: 'An error occurred while loading the account page',
      });
  }
};
...
const changePassword = async (req, res) => {
  try {
    ...
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ success: false, msg: 'Server error while changing password' });
  }
};
...
const forgotPassword = async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      message:
        'Password reset link sent to your email. Check your email for the link.',
    });
  } catch (error) {
    console.error(error);
...
const resetPassword = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(StatusCodes.NOT_FOUND)
        .json({ success: false, message: 'Password reset token not found' });
    }
