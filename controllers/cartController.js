...
const addToCart = async (req, res) => {
  try {
    ...
    if (
      !product.pricingAndAvailability ||
      typeof product.pricingAndAvailability.stockAvailability !== 'number' ||
      product.pricingAndAvailability.stockAvailability < quantity
    ) {
      return res
        .status(StatusCodes.BAD_REQUEST)
    ...
    if (cartItemIndex !== -1) {
      ...
      if (newQuantity > 5) {
        return res
          .status(StatusCodes.BAD_REQUEST)
          .json({ success: false, message: 'Maximum quantity per item is 5' });
      }
    ...
    const totalPrice = cart.items.reduce((total, item) => {
      return (
        total +
        (item.product.pricingAndAvailability.salesPrice ||
          item.product.pricingAndAvailability.regularPrice) *
          item.quantity
      );
    }, 0);
    ...
    if (itemIndex > -1) {
      if (quantity > 5) {
        return res
          ...
          .json({
            success: false,
            message: 'Quantity cannot exceed 5 for this item',
...
const checkout = async (req, res) => {
  try {
    let userId = req.session.user?._id;
    const addressId = await Address.findOne({ userId });
    ...
    const items = await Promise.all(
      cart.items.map(async (item, index) => {
        ...
        const price = getValidPrice(
          productWithOffers.discountedPrice || productWithOffers.regularPrice
        );
