...
const referralOfferController = {
  getReferralOffersPage: async (req, res) => {
    try {
      ...
      res.render('admin/referral-offer-list', {
        referralOffers,
        getStatusBadge,
      });
    } catch (error) {
  ...
  createReferralOffer: async (req, res) => {
    ...
    try {
      const { offerName, refereeAmount, referrerAmount, startDate, endDate } =
        req.body;
      ...
      res
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
        .json({
          message: 'Error creating referral offer',
          error: error.message,
        });
    }
  },
    ...
    try {
      ...
      if (!offer) {
        return res
          ...
          .json({ message: 'Referral offer not found' });
      }
      res.json(offer);
    } catch (error) {
      res
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
        .json({
          message: 'Error fetching referral offer',
          error: error.message,
        });
    }
  },
  ...
  updateReferralOffer: async (req, res) => {
    ...
    try {
      ...
      res
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
        .json({
          message: 'Error updating referral offer',
          error: error.message,
        });
    }
  },
  ...
  deleteReferralOffer: async (req, res) => {
    try {
      ...
      res
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
        .json({
          message: 'Error deleting referral offer',
          error: error.message,
        });
    }
  },
