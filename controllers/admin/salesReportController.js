...
const getSalesReport = async (req, res) => {
  try {
    ...
    res
      ...
      .json({ message: 'Error rendering sales report page' });
  }
};
...
function buildDateQuery(startDate, endDate, reportType) {
  ...
  return momentPeriod
    ? { createdAt: { $gte: moment().startOf(momentPeriod).toDate() } }
    : {};
}
...
function generateReportData(orders) {
    ...
    order.products.forEach((item) => {
      if (item.product) {
        ...
        if (item.product.category && item.product.category.length > 0) {
          ...
          report.salesByCategory[categoryId].revenue +=
  ...
  report.salesByProduct = Object.values(report.salesByProduct).sort(
    (a, b) => b.revenue - a.revenue
  );
  report.salesByCategory = Object.values(report.salesByCategory).sort(
    (a, b) => b.revenue - a.revenue
  );
