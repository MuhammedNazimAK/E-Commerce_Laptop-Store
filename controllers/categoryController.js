...
const softDeleteCategory = async (req, res) => {
  ...
  try {
    ...
    const message = category.isBlocked
      ? 'Category successfully restored'
      : 'Category successfully deleted';
    return res
      .status(StatusCodes.OK)
      .json({ success: true, message, isBlocked: updatedCategory.isBlocked });
  } catch (error) {
...
const categoryOfferController = {
  ...
  createCategoryOffer: async (req, res) => {
    ...
    try {
      const { offerName, category, discountPercentage, startDate, endDate } =
        req.body;
      ...
      if (overlappingOffer) {
        return res
          .status(StatusCodes.BAD_REQUEST)
          .json({
            message: 'An overlapping offer already exists for this category',
          });
      }
      ...
      res
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
        .json({
          message: 'Error creating category offer',
          error: error.message,
        });
    }
  },
  ...
  getCategoryOfferById: async (req, res) => {
    try {
      const offer = await CategoryOffer.findById(req.params.id).populate(
        'category',
        'name'
      );
      if (!offer) {
        return res
          .status(StatusCodes.NOT_FOUND)
          .json({ message: 'Category offer not found' });
      }
      res.json(offer);
    } catch (error) {
      res
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
        .json({
          message: 'Error fetching category offer',
          error: error.message,
        });
    }
  },
...
const loadAddCategoryOfferPage = async (req, res) => {
  try {
    ...
    res
      .status(StatusCodes.OK)
      .render('admin/category-offer-add', { categories });
  } catch (error) {
