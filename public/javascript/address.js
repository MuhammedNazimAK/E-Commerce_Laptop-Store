document.addEventListener('DOMContentLoaded', () => {
    ...
    if (e.target.classList.contains('edit-address')) {
      ...
      try {
        const response = await axios.get(
          `/my-account/edit-address/${addressId}`
        );
  ...
  AddressForm.addEventListener('submit', async (e) => {
    ...
    const isPinCodeValid = validatePinCodeInput(
      'editPinCode',
      'editPinCodeError'
    );
  ...
  addAddressForm.addEventListener('submit', async (e) => {
    ...
    if (validateAllInputs()) {
      const submitButton = addAddressForm.querySelector(
        'button[type="submit"]'
      );
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete(
            `/my-account/delete-address/${addressId}`
          );
          if (response.data.success) {
            showSuccess(
...
function updateAddressCard(address) {
  const addressCard = document
    .querySelector(`[data-id="${address.addressId}"]`)
    .closest('.address-card');
  if (addressCard) {
    addressCard.querySelector('h5').textContent = address.name;
    addressCard.querySelector('p:nth-child(2)').innerHTML =
      `<strong>Type:</strong> ${address.addressType}`;
    addressCard.querySelector('p:nth-child(3)').innerHTML =
      `<strong>City:</strong> ${address.city}`;
    addressCard.querySelector('p:nth-child(4)').innerHTML =
      `<strong>Landmark:</strong> ${address.landMark}`;
    addressCard.querySelector('p:nth-child(5)').innerHTML =
      `<strong>State:</strong> ${address.state}`;
    addressCard.querySelector('p:nth-child(6)').innerHTML =
      `<strong>Pin Code:</strong> ${address.pinCode}`;
    addressCard.querySelector('p:nth-child(7)').innerHTML =
...
function removeAddressCard(addressId) {
  const addressCard = document
    .querySelector(`[data-id="${addressId}"]`)
    .closest('.col-md-6');
  if (addressCard) {
    addressCard.remove();
  }
