...
async function prepareFormData() {
  ...
  formData.append(
    'productId',
    document.getElementById('update').dataset.productId
  );
  ...
  if (originalImageBlobs.length > 0) {
    originalImageBlobs.forEach((blob, index) => {
      const file =
        blob.file instanceof File
          ? blob.file
          : new File([blob.file], `image_${index}.jpg`, { type: 'image/jpeg' });
  ...
  const existingImageUrls = Array.from(existingImages).map(
    (image) => image.dataset.imgSrc
  );
...
async function handleUpdateClick(event) {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      showErrorMessage(
        response.data.message || 'There was an error updating the product.'
      );
    }
  } catch (error) {
  ...
  function updateFileInput() {
    ...
    originalImageBlobs.forEach((blob) => {
      const file =
        blob.file instanceof File
          ? blob.file
          : new File([blob.file], 'image/jpg', { type: 'image/jpeg' });
