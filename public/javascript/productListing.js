  ...
  if (document.querySelector('#product-container')) {
    class ProductData {
      ...
      initEventListeners() {
        ...
        document
          .querySelectorAll(
            '.brand-filter, .category-filter, .ram-filter, .processor-filter, .graphics-card-filter'
          )
          .forEach((checkbox) => {
        ...
        const newArrivalsFilter = document.getElementById(
          'new-arrivals-filter'
        );
      ...
      updateFilters() {
        ...
        this.filters.processors = [
          ...document.querySelectorAll('.processor-filter:checked'),
        ].map((el) => el.value);
        this.filters.graphicsCards = [
          ...document.querySelectorAll('.graphics-card-filter:checked'),
        ].map((el) => el.value);
        this.currentPage = 1;
        this.fetchProducts();
      }
      ...
      async fetchProducts() {
        try {
          const response = await fetch(
            `/productListing/search-and-sort?=${Date.now()}`,
            {
              method: 'POST',
              headers: {
      ...
      updateProductContainer(products) {
        ...
        products.forEach((product) => {
          ...
          const truncatedDescription = truncateString(
            product.basicInformation.description,
            150
          );
          ...
          const discountedPrice =
            product.pricingAndAvailability.salesPrice || originalPrice;
