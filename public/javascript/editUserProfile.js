document.addEventListener('DOMContentLoaded', function () {
  ...
  function handleErrorMessage(errorMsg) {
    switch (errorMsg) {
      ...
      case 'New password must be at least 6 characters long and contain at least one uppercase letter, one lowercase letter, and one number':
        newPasswordError.textContent =
          'Your new password must be at least 6 characters long and contain at least one uppercase letter, one lowercase letter, and one number.';
        newPasswordError.style.display = 'block';
        break;
      default:
        showFlashMessage(
  ...
  function showFlashMessage(type, message) {
    const flashContainer = document.querySelector(
      '#changePasswordModal .flash-message-container'
    );
