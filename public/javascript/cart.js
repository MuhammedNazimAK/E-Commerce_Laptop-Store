  ...
  try {
    ...
    if (response.data.success) {
      const price = parseFloat(
        row.querySelector('.product-price').textContent.replace('₹', '')
      );
...
async function viewAvailableCoupons() {
  try {
    ...
    if (response.data.success) {
      const couponsHtml = response.data.coupons
        .map((coupon) => `<p>${coupon.code}: ${coupon.description}</p>`)
        .join('');
...
function updateCartTotals(discount = 0, cartTotal = null) {
  ...
  const shipping =
    parseFloat(
      document.getElementById('shipping').textContent.replace('₹', '').trim()
    ) || 0;
