  ...
  return new Promise((resolve, reject) => {
    ...
    addTable(
      ...
      [
        ...
        [
          'Average Order Value',
          `${(report.totalSales / report.ordersCount).toFixed(2)}`,
        ],
      ],
      borderColor
    );
...
function addTable(doc, data, borderColor) {
  ...
  data.forEach((row, rowIndex) => {
    ...
    row.forEach((cell, columnIndex) => {
      ...
      doc
        .fillColor(textColor)
        .text(cell, x + cellPadding, y + cellPadding, textOptions);
    });
    doc.moveDown(0.5);
  });
  doc.moveDown();
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      doc
        ...
        .text(
          `Invoice Date: ${moment(order.createdAt).format('MMMM D, YYYY')}`,
          40,
          110
        )
        .text(
          `Due Date: ${moment(order.createdAt).add(30, 'days').format('MMMM D, YYYY')}`,
          40,
          125
        );
      ...
      doc
        ...
        .fontSize(10)
        .fillColor(textColor)
        .text(`${order.subtotal.toFixed(2)}`, totalsLeft + 120, totalsTop, {
          align: 'right',
        })
        .text(`${order.gst.toFixed(2)}`, totalsLeft + 120, totalsTop + 15, {
          align: 'right',
        })
        .text(
      ...
      doc
        ...
        .fontSize(12)
        .fillColor('#FFFFFF')
        .text(`${order.total.toFixed(2)}`, totalsLeft + 120, totalsTop + 65, {
          align: 'right',
        });
      ...
      doc
        ...
        .text(
          '2. Prices are subject to change without notice. Please confirm pricing before placing your order.',
          40,
          doc.y + 5
        )
        .text(
          '3. You may return items within 7 days of delivery for a full refund if they are in original condition.',
          40,
          doc.y + 5
        )
        .text(
          '4. Warranty claims must be submitted within the product warranty period.',
          40,
          doc.y + 5
        )
        .text(
          '5. Laptop Store is not responsible for any indirect damages arising from the use of our products.',
          40,
          doc.y + 5
        )
        .text(
          '6. For any assistance, please contact our support at support@laptopstore.com.',
          40,
          doc.y + 5
        );
...
function createTable(doc, y, headers, data) {
  ...
  headers.forEach((header, i) => {
    ...
    doc.text(
      header,
      45 + columnWidths.slice(0, i).reduce((a, b) => a + b, 0),
      currentY + 5,
      {
