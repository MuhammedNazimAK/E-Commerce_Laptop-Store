...
(function (global, factory) {
  ...
  function native(a, b) {
    let c;
    typeof a === 'string' && ((b = a), (a = document));
    for (let d = 0; d < SUBST.length; ++d) {
      b = b.replace(SUBST[d][0], SUBST[d][1]);
      for (let e = 0; e < VENDOR_PREFIXES.length; ++e)
        if (
          ((c = VENDOR_PREFIXES[e]),
          (c += e === 0 ? b : b.charAt(0).toUpperCase() + b.substr(1)),
          defined(a[c]))
        )
          return a[c];
    }
  ...
  var SUBST = [
    ...
    const fsEnabled = native('fullscreenEnabled');
    const parsedChromeUA = ua.match(/Android.*Chrome\/(\d+)\./);
    const IS_ANDROID_CHROME = !!parsedChromeUA;
    let CHROME_VERSION;
    let ANDROID_CHROME_VERSION;
  IS_ANDROID_CHROME && (ANDROID_CHROME_VERSION = parseInt(parsedChromeUA[1]));
  const IS_NATIVELY_SUPPORTED =
      (!IS_ANDROID_CHROME || ANDROID_CHROME_VERSION > 37) &&
      defined(native('fullscreenElement')) &&
      (!defined(fsEnabled) || fsEnabled === !0);
    const version = $.fn.jquery.split('.');
    const JQ_LT_17 = parseInt(version[0]) < 2 && parseInt(version[1]) < 7;
    const FullScreenAbstract = function () {
      ((this.__options = null),
  ...
  FullScreenAbstract.prototype = {
    ...
    _saveAndApplyStyles: function () {
      const a = $(this._fullScreenElement);
      this.__savedStyles = {};
      for (const b in this.__options.styles)
    ...
    _revertStyles: function () {
      ...
      (a.is('body') &&
        ...
        this.__options.toggleClass &&
          a.removeClass(this.__options.toggleClass));
    },
    open: function (a, b) {
  ...
  const FullScreenNative = function () {
  ...
  extend(FullScreenNative, FullScreenAbstract, {
    ...
    open: function (a, b) {
      ...
      const c = native(a, 'requestFullscreen');
      c.call(a);
    },
    exit: $.noop,
  ...
  const FullScreenFallback = function () {
  ...
  (extend(FullScreenFallback, FullScreenAbstract, {
    ...
    __delegateKeydownHandler: function () {
      ...
      const b = JQ_LT_17 ? a.data('events') : $._data(document).events;
        const c = b.keydown;
      JQ_LT_17
    ...
    __keydownHandler: function (a) {
      return !this.isFullScreen() || a.which !== 27 || (this.exit(), !1);
    },
    _revertStyles: function () {
